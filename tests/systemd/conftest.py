...
# `inside/` is made uncollectable rather than deselected by marker, so that a bare
# `uv run pytest -m systemd` at the repo root is exactly the driver. The inner modules import
# `audera.services.system` and call it for effect, so collecting them on a developer's machine would
# mean a host-side pytest issuing `systemctl` against the developer's own init system.
#
# The directory is ignored rather than its contents: `'inside/*'` ignores the modules but pytest still
# descends into the directory to load its `conftest.py`, and that file reads the `/app/os/dietpi/lib/`
# shell libraries at import, a path that exists only in the container.
...
def pytest_collection_modifyitems(config, items):
    ...
    Applies to the driver and to the inner modules alike, so the driver's inner command passes
    `-m systemd`: `addopts` deselects the marker by default, and an inner run without the override
    would collect five modules and select none of them.

    The path test is required. A `conftest.py` hook is registered against the whole session rather
    than its own directory, so `items` is every test pytest collected; marking unconditionally would
    put `systemd` on all four hundred and leave a bare `uv run pytest` selecting none of them.
    """
...
def _wait_for_systemd(container, timeout: float = _BOOT_TIMEOUT) -> None:
    ...
    while `is-system-running` is the manager answering for itself. The exit status is ignored because
    it is non-zero for `degraded`, the expected steady state here.
    """
...
def systemd_image():
    ...
    order makes a source-only rebuild seconds. buildx caching is not available here: `DockerImage`
    uses docker-py's legacy build endpoint and would have to be replaced first.
    """
...
def systemd_container(systemd_image):
    ...
    The suite's other container fixtures yield a `(host, port)` because the service under test is
    reached over a socket. Here the code under test runs inside the container, so the interface is
    `container.exec` and the handle is what the fixture yields.

    The keyword arguments are one `with_kwargs` call because it replaces rather than merges:

    - `privileged` and a read-write `/sys/fs/cgroup` are what let systemd manage cgroups. On an
      ephemeral GitHub-hosted VM that is safe; on a self-hosted runner it grants container root, and
    ...
    - `extra_hosts` resolves `plex.tv` locally, so `plexamp.service` runs verbatim instead of
      needing a container-only drop-in over the step under test. Its `ExecStartPre` polls
      `getent hosts plex.tv` for up to 60 s otherwise. This is a run argument rather than a
      Dockerfile line because Docker bind-mounts `/etc/hosts` at container start, which discards
      anything the image wrote there.
    """
