...
drives the same handlers from the page with `system.systemctl` stubbed out, so it covers what the
operator sees and what reaches `~/.audera` and `snapserver.conf`. Nothing there can show a backend
Snapserver forked and never reaped, which keeps running with every one of those assertions green.

`_enable_source` and `_disable_source` are awaited as written rather than re-driven step by step,
because the ordering is under test.
...
`toggle.apply` clears its counters against. Both sit beside the choreography rather than beside the
seam: one manager default costs this module's disable path aborting before the restart, and the other
costs a toggle refused outright with Snapserver left dead, reachable only because every toggle restarts
it.
"""
...
# Read from the catalog rather than restated. A source's id, label and units are defined once, in
...
# The suffixes systemd renders a timespan with, and what each is worth in seconds. There is no bare
# minutes suffix to disambiguate from milliseconds.
_SCALE = {'us': 1e-6, 'ms': 1e-3, 's': 1.0, 'min': 60.0, 'h': 3600.0}

# How many raw restarts `_wedge_start_limit` will spend looking for the manager's refusal. The default
# burst is five and the `provisioned` fixture has already spent part of it, so the refusal normally
# lands well inside this; the bound stops a manager with no reachable limit looping.
...
# systemd's start limit reachable. One a second is conservative for a switch.
...
# under a second even on the device.
...
def slow_conf_write(monkeypatch) -> None:
    ...
    `toggle.apply` reads `conf.render_snapserver` through the module at call time, so delaying the
    render delays the write and nothing else. Neither the restart nor the units move, and the conf that
    lands is byte-identical to the undilated one.

    Correctly ordered, the write completes before the restart that reads it and the delay is only
    wall-clock. Reordered, the restarted Snapserver opens the file mid-write: `open(…, 'w')` truncates
    before the render it dilates, so Snapserver finds an empty configuration and falls back to a single
    built-in `default` stream. The dilation removes the millisecond race that decides whether that or a
    stale conf is what it reads.
    """
...
def wedged_nqptp() -> Iterator[str]:
    ...
    clears the list systemd already parsed, which is the only way to replace rather than append to it.

    `nqptp` is the unit to wedge because it is the one an operator's toggle stops.
    """
    ...
    # The `stop` is unconditional because the test may have failed before its own `disable --now`, and
    # a surviving SIGTERM-proof process would outlive every later test in this module.
...
def _seconds(timespan: str) -> float:
    ...
    `systemctl show` renders these properties rather than emitting the microseconds their names promise,
    so a comparison against `system.TIMEOUT` has to go through this. It raises on a string it read no
    component out of, because an unparseable value summing to `0.0` would satisfy every
...
def _manager_default(name: str) -> str:
    ...
    `systemctl show` with no unit answers for the manager, a different scope from `unit_state`'s. It
    `check`s, because a manager that cannot state its own configuration must not be read out of an
    empty string.
    """
...
def _wedge_start_limit(unit: str) -> float:
    ...
    when a test begins: the `provisioned` fixture starts Snapserver twice inside its own setup.

    The counter is a trailing window, so the wedge is not permanent: once the oldest of the counted
    starts ages out, the unit becomes startable again with nothing having repaired it. A caller that
    took that long to reach its assertion would pass whether or not the code under test cleared a
    counter, so it has to fail as inconclusive instead.

    The deadline is one interval from before the first restart rather than from the refusal, which is
    the tighter of the two, since the priming restarts take about a second. It bounds the harness's
    pacing rather than proving anything, since starts the `provisioned` fixture already put in the
    window are older still; a handler call is a second or two against a ten-second interval.
    """
...
def _conf_stream_ids() -> list[str]:
    ...
    wrote the file, so rendering again and diffing would assert only that a pure function is
    deterministic. `name` is the stream id per `CATALOG`'s rule 1, so this is directly comparable with
    `stream_status()`'s keys.
    """
    ids: list[str] = []
...
def _failures(notifications: list[tuple[str, str]]) -> list[str]:
    ...
    without this a `systemctl` that refused the job surfaces as a missing process with systemd's reason
    nowhere in the failure message.
    """
...
async def test_enabling_spotify_makes_snapserver_fork_its_backend(page, notifications):
    ...
    The assertion is on the parent rather than the existence of the process. go-librespot under
    Snapserver's `MainPID` is a backend Snapserver owns and will reap; the same process under pid 1 is
    the orphan `os/dietpi/AGENTS.md` records, which would satisfy a bare "is it running" check.

    `await_pids` rather than `pids_for`, because `restart` returns once systemd has forked Snapserver
    and Snapserver forks its backends later still. The stream registration is polled for the same
    reason, through Snapserver's own answer rather than the UI's reading of it.
    """
...
async def test_disabling_spotify_reaps_the_backend_snapserver_forked(page, notifications):
    ...
    Dropping that restart from the disable path leaves go-librespot running against a conf that no
    longer names it, which every argv assertion in `tests/ui/test_streamer.py` accepts. Here it fails on
    `still_alive`.

    Zombies are asserted separately from orphans because the two need different fixes: an orphan steals
    CPU from playback, sync and DSP on a Pi Zero 2 W, while a zombie consumes a pid-table slot its
...
async def test_disabling_a_source_leaves_no_process_from_its_units(page, notifications, source):
    ...
    enabled source, and Spotify has no units, so it cannot contribute a process to the trees below. When
    the parameter is AirPlay, which the baseline already enabled, the enable below is skipped.

    The process tree rather than a pattern match, because `plexamp.service` is `bash -c '… exec node …'`:
    its `ExecStart` path is an interpreter half the container shares, while the process that leaks is
    ...
    The manager's bookkeeping is asserted alongside the process table because the two can disagree:
    systemd forgets a unit it could not stop, and the backend keeps running under pid 1 with nothing
    naming it.
    """
    await index._enable_source(page, SPOTIFY)
...
async def test_disabling_a_source_moves_its_listeners_to_the_chosen_destination(page, notifications, listening_player):
    """`_reassign_groups` moves a real group, which is what distinguishes it from a no-op.

    The other tests here pass `destination=None`, and without `listening_player` there is no client, so
    `get_groups()` answers `[]` and a destination that never reached `Group.SetStream` looks identical
    to one that did.

    PlexAmp is the destination rather than AirPlay so that Snapserver's own fallback cannot stand in for
    the handler's work. Per `catalog.py`'s rule 3, a group whose stream the conf stops providing is
    reassigned to `default_source`, which is AirPlay here, so asking for AirPlay would pass even if
    `Group.SetStream` was never reached. Measured with that call removed, this reads `Spotify` and
    fails.

    It also covers the id-versus-label class by effect: `Group.SetStream` takes the stream name, which
    is the source *id*, and sending the label `'Spotify Connect'` mis-routes with no error.
    """
    await index._enable_source(page, SPOTIFY)
...
async def test_the_running_streams_match_the_conf_on_disk(page, notifications, slow_conf_write):
    ...
    status from Snapserver over its own client.
    ...
    container almost every time, while a Pi Zero 2 W loses it. The fixture widens the interval until the
    outcome is decided by the order alone. Measured both directions: reordered and dilated, this fails
    on the stream set; correctly ordered and dilated, it passes.

    Asserted after an enable and again after a disable, because the two write the conf from different
    sets, `set_enabled`'s return in one direction and the remainder in the other, so a stale read-back
    would show on only one of them.
    """
    await index._enable_source(page, SPOTIFY)
...
async def test_the_last_enabled_source_is_refused_without_touching_the_host(page, notifications):
    """Refusing the last enabled source leaves the conf and every unit untouched.

    `CATALOG`'s rule 2 gives the reason: `getDefaultStream()` returns `nullptr` for an empty stream list
    and Snapserver dereferences it at the first client connect, so a zero-stream conf crashes the
    server. `render_snapserver()` raises rather than emitting one, so a refusal that reached the conf
    write would fail loudly; the failure this asserts against is the quieter one where the units move
    first and the render then raises.

    The conf's `st_mtime_ns` rather than its contents, since a rewrite of identical bytes is still a
    rewrite of `/etc/`, and `nqptp`'s `MainPID` alongside Snapserver's because the units are the step
    that would run before the render raised.
    """
...
def test_the_manager_default_would_exceed_the_seams_budget():
    """The manager's default stop timeout exceeds `system.TIMEOUT`, so `streamer.sh`'s override matters.

    Without this, the assertions above are satisfiable by a manager whose default already fits. The
    default is six times the seam's budget.
...
def test_a_wedged_backend_is_killed_inside_the_budget(wedged_nqptp):
    ...
    Guards a measured defect. A backend that ignored SIGTERM made `systemctl disable --now` outlive the
    ...
    `restart snapserver` after it never ran, leaving the conf and the running server divergent.
    ...
    escalate to SIGKILL, since a stub that died on SIGTERM would return in milliseconds. The lower bound
    is read from the unit rather than restated.

    The process table is checked afterwards for the reason `test_stop_leaves_no_process_and_no_zombie`
    gives: a SIGKILL systemd did not wait for leaves the backend alive and the unit `inactive`, and
    `MainPID=0` would report that as a clean stop.

    The unit ends `failed` rather than `inactive`, which is systemd recording that it had to force the
    stop. Both `_plexamp_state` and the Sources tab go through `is_active`, which answers False for
    `failed`, so a forced stop presents as "not running" rather than as an error the operator must clear.
    """
...
def test_snapserver_takes_the_managers_start_limit():
    ...
    `snapserver.service` states no `StartLimit*` of its own and so inherits the manager's, which records
    the rejected alternative: clearing the counter was chosen over `StartLimitIntervalSec=0` on the unit,
    and a later change taking the other route would leave `toggle.apply`'s `reset-failed` a superstition
    with nothing failing. `camilladsp.service` already carries that line.

    Reachability is a rate rather than a count. One toggle is one restart, so
    `StartLimitBurst / StartLimitIntervalSec` is the sustained toggles per second systemd will accept,
    and against a manager whose default were in the hundreds the `reset-failed` would be moot. The size
    of `CATALOG` does not bound it, since a toggle need not be distinct: flipping one source off and on
    is two.
    """
...
async def test_a_toggle_revives_a_snapserver_the_start_limit_wedged(page, notifications):
    ...
    manager's five starts per ten seconds, and the sixth toggle inside that window is refused. The unit
    is then `failed`, port 1780 is closed, every source chip reads down, and audio has stopped, with no
    retry since `Restart=on-failure` is what the limit refused.

    Driven from an already-wedged server rather than by six toggles in a row, because the six-toggle
    version can go vacuous: if the handlers are slow enough that the calls straddle the interval, the
    counter ages out and the last toggle is accepted for an unrelated reason. `_wedge_start_limit`
    confirms `start-limit-hit` before returning, and the deadline it returns fails the test as
    inconclusive if the assertion arrives after the window.
...
async def test_a_toggle_starts_a_source_unit_the_start_limit_wedged(page, notifications):
    ...
    `reset-failed` covers `(*source.units, 'snapserver')` rather than just the server, because the units
    are startable from the tab too: PlexAmp is one switch and two units, so toggling it off and on three
    times is six starts each. Audera does not own `nqptp`'s unit file, so a per-unit
    `StartLimitIntervalSec=0` could not have covered it.

    PlexAmp rather than AirPlay because the baseline leaves its units disabled and stopped, so the enable
    below is a real `enable --now` against a unit systemd is refusing to start. AirPlay is already
    running, and a `reset-failed` on a running unit would show nothing.
    """
