...
The rest of the suite reaches `@platform.requires('dietpi')`'s passing branch only by
`monkeypatch.setattr(platform, 'NAME', 'dietpi')`, which pins that the decorator reads the constant
and nothing about the constant being derivable on a real device. Here the derivation itself is under
test.
"""
...
def test_systemd_is_pid_1():
    ...
    this directory would still pass: a stopped unit and an unmanaged one look alike to anything that
    only reads argv.
    """
...
def test_the_platform_resolves_to_dietpi_without_being_patched():
    ...
    is in the shape `dotenv` can read. A file present but unparseable would leave `NAME` at `'linux'`
    and turn every gated call in this directory into a `RuntimeError`.
    """
...
def test_a_gated_function_runs_instead_of_raising():
    ...
    `is-system-running` is the cheapest gated call that answers, since it needs no unit. It exits
    non-zero for a `degraded` manager, so `check=False` keeps the answer readable rather than an
    exception, and the assertion is on the output rather than the status.
    """
...
def test_the_image_ships_no_audera_unit_files():
    """Provisioning is this lane's fixture, which only holds if the image is empty of units.

    `write_streamer_units()` installs every unit the other modules assert on, so a unit baked into the
    image would be one the tests read but provisioning never wrote. `/etc/systemd/system/` is Audera's
    alone here; the units the device gets from apt are in `/usr/lib/systemd/system/`.
    """
