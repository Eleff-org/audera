...
def test_single_band_matches_eval_filter_math():
    ...
    config = DSPConfig(player_id='x', preamp_db=-3.0, bands=[band])
...
def test_single_peaking_band_peaks_near_gain():
    band = Band(id='b1', type='Peaking', freq=1000.0, gain=6.0, q=1.0)
    config = DSPConfig(player_id='x', bands=[band])
...
def test_preamp_shifts_peak_by_scalar():
    band = Band(id='b1', type='Peaking', freq=1000.0, gain=6.0, q=1.0)
    flat = response_peak_db(DSPConfig(player_id='x', preamp_db=0.0, bands=[band]))
    shifted = response_peak_db(DSPConfig(player_id='x', preamp_db=-4.0, bands=[band]))
...
def test_empty_config_returns_preamp():
    assert response_peak_db(DSPConfig(player_id='x', preamp_db=-2.0)) == pytest.approx(-2.0)


def test_response_curve_lengths_match_and_frequency_is_strictly_increasing():
    band = Band(id='b1', type='Peaking', freq=1000.0, gain=6.0, q=1.0)
    frequencies, magnitudes = response_curve(DSPConfig(player_id='x', bands=[band]))
...
def test_response_curve_no_enabled_bands_synthesizes_full_flat_line():
    ...
    config = DSPConfig(player_id='x', preamp_db=-3.0, bands=[Band(id='b1', freq=1000.0, gain=6.0, enabled=False)])
...
def test_response_peak_db_equals_curve_max_on_fine_grid():
    band = Band(id='b1', type='Peaking', freq=1000.0, gain=6.0, q=1.0)
    config = DSPConfig(player_id='x', preamp_db=-2.0, bands=[band])
