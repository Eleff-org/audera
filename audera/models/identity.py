...
def generate_uuid_from_mac_address(mac_address: str) -> str:
    ...
    """
    return str(uuid.uuid3(namespace=uuid.NAMESPACE_DNS, name=mac_address.replace(':', '')))
...
class Identity:
    ...
    def to_dict(self):
        ...
        return {'name': self.name, 'uuid': self.uuid, 'mac_address': self.mac_address, 'address': self.address}
    ...
    def __eq__(self, compare):
        ...
        if isinstance(compare, Identity):
            return self.mac_address == compare.mac_address and self.address == compare.address
