...
A new `FEATURES` entry names every option with an `FF_*` constant (ADR 004 §2), so a flag-gated branch reads as flag-gated at the point of use; `player_grouping` and `dsp_band_editor` follow this. `player_selection` and `volume` predate the ADR and still use bare literals; retrofitting them belongs in a separate mechanical commit.

The Sources tab has no `FEATURES` entry. Its two candidate options, the "at least one source stays enabled" guard and reassigning listeners before their stream stops existing, are correctness guards: a zero-stream `snapserver.conf` crashes Snapserver at the first client connect, and a stream removed under a live group mis-routes it with no error. Neither is offered as an option.
...
- A source's id, label, URI, and units are not restated in the UI. A control offering destinations maps id → label, renders the label, and sends the id to `Group.SetStream`; the two differ (`Spotify` vs. `Spotify Connect`), and sending the label mis-routes with no error. The id is the Snapcast stream name, stated once in `catalog.py`'s rule 1; see the root `AGENTS.md` § Audio sources for the source/stream split these two tabs sit either side of.
- Dispatch per-source variation through a table keyed on the catalog's discriminator rather than an `if` chain. `index._SETUP_FLOWS` maps `SourceDefinition.setup` to a `(state, build)` pair, so the card builder does not special-case `'plex_claim'` and a second flow is a one-row diff. An unrecognised discriminator fails open, showing the stream's real status instead of leaving the card in `setup required` indefinitely; an exception from a flow that does exist fails closed on `_SETUP_REQUIRED`.
- A flow reports a chip label rather than a boolean. `state()` returns `str | None`: `None` once setup is done, otherwise the word the chip shows. A flow can have more than one incomplete state and they do not all read the same. PlexAmp has two: `setup required` for a device that was never claimed, and `starting` for one that systemd has started but that has not bound its port yet. The card builder resolves the label once and passes it to both the chip and `build(page, pending)`, so the two cannot disagree within a render and the panel switches on the same word the chip is showing. The label is the flow's own constant; `index` never learns what `'starting'` means, which also keeps `_plex` from importing from `index`.

The tab is not polled. `index.render`'s 10 s timer refreshes the Players tab only: polling the Sources tab would put `get_stream_status()`, and with PlexAmp enabled a `systemctl is-active`, on a 10 s loop, and would cancel the PlexAmp claim flow's own timers every tick. The chips are accurate at load, after each toggle, at claim completion, and on re-entry to the tab; each of those paths refreshes explicitly.

`_plex._build_starting_panel` is the one exception, and its poll is bounded. The operator takes no action during a startup window, so no other path would refresh a card that came up seconds after the page loaded, and it would read `starting` until the tab was left and re-entered. The poll terminates: `_plexamp_state` stops answering `'starting'` once the port opens or `STARTUP_GRACE` passes, and the refresh that replaces the panel deletes the timer with it. Anything else added here needs the same two properties: a state the poll cannot observe otherwise, and a condition that ends it.

Re-entry keeps an unpolled tab from staying stale until the next reload. NiceGUI builds every tab panel eagerly at page load, so a status word that changed while the operator was on another tab has no other path to update. `index.render`'s `tabs.on_value_change` handler performs one read per return, bounded by the operator rather than by a timer, and carries two refusals:
...
- A claim in flight refuses the repaint, with no notification. As with the mid-claim toggle refusal, a repaint deletes the claim flow's elements and cancels its timers. There is no notification because returning to a tab is not a request to abandon what is on it.
...
- Every read goes off-thread, runs concurrently, and is bounded, because the page builder is `async`. Both service clients connect with a 5 s `open_timeout`, and the builder runs on the event loop that serves every session, so one unreachable host previously held the whole UI and the 10 s poll re-entered the same wait. The page uses `asyncio.to_thread` per read, `asyncio.gather` per round, and `index._READ_TIMEOUT` over each round. The ceiling is there for NiceGUI: an `async` page builder that has not returned by `response_timeout` (3 s, and this page takes the default) is cancelled and its client deleted, so overrunning renders no page at all rather than a degraded tab. A synchronous builder could block indefinitely without tripping that deadline, so going `async` introduced this deadline. `wait_for` cancels the await rather than the worker thread, which keeps blocking until its own `open_timeout`; the result is an orphaned thread that nothing waits on and an event loop that stays unblocked.
- An `async` `@ui.refreshable` is not re-entrant, where a synchronous one is. `_execute_refresh` clears the target's container and then runs the builder: inline when it is sync, so clear and build are one uninterrupted call, but as a background task when it is `async`. Two refreshes in the same tick therefore both clear (the second over an empty container) and then both append, so every element renders twice. The 10 s poll and any handler that refreshes are such a pair. `build_players_tab` stamps `Page._players_generation` before its first `await` and returns at each resumption point where a newer build has since started, so the superseded build renders nothing and skips the reads it had not yet made. Anything else made `async` behind a refreshable needs the same guard.
- The volume slider is seeded at construction and never written to afterwards. `on_change` is its write path to CamillaDSP and to Snapcast's mute state, so a programmatic `set_value()` is indistinguishable from a drag: it pushes volume to the hardware and clears mute. Constructing the element with a `value=` does not fire it, and the seed is also step-aligned to the integer slider, since an unaligned one fires a phantom `update:model-value` on the next refresh. A fresher reading therefore reaches the UI by rebuilding the card rather than by updating it in place. That rules out "fetch after first paint, fill the sliders in" as a way to move the volume read out of the render's deadline: the fill writes back on every poll. The test that catches it is `test_players_tab_disabled_experience_toggle_off_mutes_client`, which touches no slider and still sees `set_client_volume`. Any scheme for deferring the volume read has to land in the cache and wait for a rebuild rather than touch a live element.
- An unreadable volume is withheld rather than defaulted. `_volumes` reports `None` and the card renders a disabled slider labelled `—`, with no Mute binding to re-enable it. It used to fall back to `DEFAULT_PERCENT_VOLUME`, which draws a slider at 25%: indistinguishable from a player genuinely at 25%, and the base a drag would then write against. Volume is the tab's only per-player, off-host read, so it fails more often than the Snapserver RPCs.
- `get_stream_status()` is read under both layouts. Liveness gates attachment as well as the by-stream header's status word: a source Snapserver is not feeding is not a destination either grouping may offer, so both pay the third RPC. If the 10 s poll ever shows up in a profile, the agreed remedy is one new client method returning clients and groups together, rather than a second payload parser in the page or a `stream_id` on `Player`, which would replicate state Snapcast owns.
- Two triggers share one menu, and only one of them is active at a time. `_build_move_menu` is single-copy; `by_stream` selects which element opens it. The chip is a body row, so a minimized card drops it; the move button is a header-row button beside DSP and settings, so a minimized card keeps it disabled. `by_stream` is consulted at two points in `_build_player_card`. `_move_refusal` is likewise shared, so both triggers refuse on the same grounds (no group, or no attachable destination) and only the rendering of the refusal differs.
- A destination Snapserver is not feeding is shown disabled beside its status word rather than dropped. An operator who expects four sources and counts three has no explanation, where a greyed row reading `not running` states the reason. The menu serves both layouts for that reason: a `ui.select` over a `dict[id, label]` cannot disable a single option, so the alternative was dropping the option with no explanation.
...
- Clear `_dialog_open` before refreshing, not after. Both triggers latch it while the menu is open so the poll cannot destroy it mid-interaction. A flag latched by an element the refresh deleted freezes the poll for the life of the page. `ui.menu.on_value_change` covers open and close in one event, where the select it replaced needed Quasar's `popup-show`/`popup-hide` pair.
- Destinations are the enabled set ∩ `CATALOG`, plus the current stream when it is not in that set. The current stream leads the list and is what the chip names; a chip reading `Unassigned` for a player parked on an uncatalogued stream would report that it is listening to nothing, which is false. `_move_destinations` then drops the current stream from the menu, since selecting it is not a move. `_sections` gives that stream its own header for the same reason the chip names it.
...
- A dev box behaves differently from the device. `sources_dal.set_enabled` is plain JSON under `~/.audera` and has no platform gate, so a source toggle on a developer's machine mutates `~/.audera/sources.json` and only then fails at the `/etc/` write or the `systemctl` call. That is the roll-forward ordering working as designed, but it means a dev-box toggle leaves the enabled set changed.
