...
VERSION = (
    '.'.join(
        [os.getenv('G_DIETPI_VERSION_CORE', '0'), os.getenv('G_DIETPI_VERSION_SUB', '0'), os.getenv('G_DIETPI_VERSION_RC', '0')]
    )
    if os.getenv('G_DIETPI_VERSION_CORE')
    else platform.version().strip().lower()
)


# Decorator function(s)
def requires(platform_: Literal['any', 'dietpi', 'windows', 'linux', 'darwin'] = 'any') -> Callable:
